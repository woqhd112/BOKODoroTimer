	...
	: m_wnd(activeDlg)
{
	...
	m_staticBrush_Green.CreateSolidBrush(DI_BK_COLOR_GREEN);
	m_staticBrush_Yellow.CreateSolidBrush(DI_BK_COLOR_YELLOW);
	m_staticBrush_Red.CreateSolidBrush(DI_BK_COLOR_RED);
...
void DlgInterface::CreateFrame(FrameDicisionType type)
{
	...
	{
		LoadPNGResource(m_pngBackground1, IDB_PNG_BK_MAIN, "PNG");
		LoadPNGResource(m_pngBackground1_Green, IDB_PNG_BK_MAIN_GREEN, "PNG");
		LoadPNGResource(m_pngBackground1_Yellow, IDB_PNG_BK_MAIN_YELLOW, "PNG");
		LoadPNGResource(m_pngBackground1_Red, IDB_PNG_BK_MAIN_RED, "PNG");
	}
	...
	{
		LoadPNGResource(m_pngBackground1, IDB_PNG_BK_SUB, "PNG");
	}
...
void DlgInterface::DrawFrame(CPaintDC* in_pDC, int colorValue)
{
	if ((m_eFrameType == FDT_MAIN_DLG) || (m_eFrameType == FDT_SUB_DLG) || (m_eFrameType == FDT_LIGHT_DLG))
	{
		...
		if (m_bMainDlg)
		{
			if (colorValue == 0)
			{
				bmp.Attach(m_pngBackground1);
			}
			else if (colorValue == 1)
			{
				bmp.Attach(m_pngBackground1_Green);
			}
			else if (colorValue == 2)
			{
				bmp.Attach(m_pngBackground1_Yellow);
			}
			else if (colorValue == 3)
			{
				bmp.Attach(m_pngBackground1_Red);
			}
		}
		else
		{
...
void DlgInterface::LoadResourceFont(int nResourceFontID)
{
	...
	HRSRC res = FindResource(hResInstanceBold,
		MAKEINTRESOURCE(nResourceFontID), "TEXT");
